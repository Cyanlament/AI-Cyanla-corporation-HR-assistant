...
void FAQWidget::setupUI()
{
    ...
    // 播放视频按钮
    m_playVideoButton = new QPushButton("播放视频");
    m_playVideoButton->setStyleSheet(R"(
        QPushButton {
    ...
    m_playVideoButton->hide(); // 初始隐藏
    connect(m_playVideoButton, &QPushButton::clicked, this, &FAQWidget::onPlayVideoButtonClicked);
    ...
    answerLayout->addWidget(m_playVideoButton);
...
void FAQWidget::loadFAQData()
{
    ...
    m_faqData.append({"部门相关", "研发部（Binah）和构筑部（Keter）有什么区别？",
                      "研发部（R&D）专注于技术本身，负责解析异常、研发新装备和新技术。\n而构筑部（Architecture）则着眼于宏观战略，负责规划整个能源提取计划的蓝图和最终目标，\n确保所有部门的工作方向一致。\n简单说，研发部思考如何实现，而构筑部定义要实现什么。", 95});
    ...
    m_faqData.append({"福利政策类", "什么是心理健康假？怎么申请？",
                      "鉴于我们工作的特殊性，公司特别设立了带薪心理健康假。\n当你感到压力过大、需要调整时，可以向福利部提出申请。\n福利部的专业人员会进行评估，批准后即可休假。\n每年最多可享受15天。", 87});
...
void FAQWidget::onFAQItemClicked(QListWidgetItem* item)
{
    ...
    if (index >= 0 && index < m_filteredData.size()) {
        ...
        // 如果有图片路径，添加图片
        if (!faq.imagePath.isEmpty()) {
            formattedAnswer += QString("<br><br><img src='%1' width='300' alt='示意图'/>").arg(faq.imagePath);
            formattedAnswer += "<br><small><i>图: 相关示意图</i></small>";
        }
        ...
        // 处理视频
        if (!faq.videoPath.isEmpty()) {
            m_currentVideoPath = faq.videoPath;
            m_playVideoButton->show();
        } else {
            m_currentVideoPath.clear();
            m_playVideoButton->hide();
        }

        // 显示反馈按钮
        m_btnHelpful->show();
        m_btnNotHelpful->show();
    }
}
...
void FAQWidget::onPlayVideoButtonClicked()
{
    if (!m_currentVideoPath.isEmpty()) {
        VideoPlayerDialog *player = new VideoPlayerDialog(m_currentVideoPath, this);
        player->setAttribute(Qt::WA_DeleteOnClose);
        player->exec();
    }
}

// 以下函数不再需要，但为了保持接口兼容性保留
void FAQWidget::onPlayButtonClicked() {}
void FAQWidget::onStopButtonClicked() {}
void FAQWidget::onMediaStatusChanged(QMediaPlayer::MediaStatus status) {}
